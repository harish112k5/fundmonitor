...
// Check project access middleware for individual routes
const checkProjectAccess = async (req, res, next) => {
  const { projectId } = req.params;
  const { user_id, role_id } = req.user;
  const allowed = await canAccessProject(user_id, role_id, projectId);
  if (!allowed) {
...
// 1. Dashboard Metrics
router.get('/dashboard/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
...
router.get('/budget/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const [budgets] = await pool.query('SELECT * FROM budget_master WHERE project_id = ?', [projectId]);
  
  if (budgets.length === 0) {
    return res.json({ success: true, data: { master: null, details: [], analysis: [] }, message: 'No budget found' });
  }
  ...
  res.json({ success: true, data: { master, details, analysis }, message: 'Budget retrieved' });
}));

router.post('/budget/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  ...
  res.json({ success: true, data: { budgetId }, message: 'Budget created' });
}));

router.get('/statements/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  ...
  res.json({ success: true, data: { pnl }, message: 'Statements retrieved' });
}));

router.get('/ratios/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const costs = await getProjectCosts(projectId);
  const revenues = await getProjectRevenues(projectId);
  const funding = await getProjectFunding(projectId);
  ...
  res.json({
    ...
    message: 'Ratios retrieved'
  });
}));

router.get('/statements/full/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const costs = await getProjectCosts(projectId);
  const revenues = await getProjectRevenues(projectId);
  const funding = await getProjectFunding(projectId);
  ...
  res.json({ success: true, data: { balanceSheet, cashFlow }, message: 'Full statements retrieved' });
}));

// 3. Forecast Endpoints
router.post('/forecast/generate', authMiddleware, asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: { id: result.insertId }, message: 'Forecast generated' });
}));

router.get('/forecast/:projectId/:type', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: { forecasts }, message: 'Forecasts retrieved' });
}));

// 4. Planning & Goals Endpoints
router.post('/plan/create-goal', authMiddleware, asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: null, message: 'Goal created' });
}));

router.get('/plan/:projectId/goals', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const [goals] = await pool.query('SELECT * FROM financial_goals WHERE project_id = ?', [projectId]);
  res.json({ success: true, data: { goals: goals[0] || null }, message: 'Goals retrieved' });
}));

// 5. Tax Compliance Endpoints
router.get('/tax/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const [taxes] = await pool.query('SELECT * FROM tax_compliance WHERE project_id = ? ORDER BY due_date ASC', [projectId]);
  res.json({ success: true, data: { taxes }, message: 'Tax compliance retrieved' });
}));

router.post('/tax/:projectId', authMiddleware, checkProjectAccess, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  ...
  res.json({ success: true, data: null, message: 'Tax record added' });
}));
