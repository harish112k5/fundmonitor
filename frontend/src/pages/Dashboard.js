        ...
        <>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', zIndex: 1 }}>
                <div style={{ 
                  ...
                  fontSize: '12px', color: '#57534E', textAlign: 'right' 
                }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '14px', marginBottom: '28px' }}>
            {statsLoading ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} style={{ background: '#1E1E1E', border: '1px solid #2D2D2D', borderRadius: '8px', padding: '18px 20px', minHeight: '100px' }}>
                  <div style={{ width: '60%', height: '10px', background: '#2D2D2D', borderRadius: '2px', marginBottom: '12px', animation: 'sitePulse 1.5s ease-in-out infinite' }} />
                  <div style={{ width: '40%', height: '24px', background: '#2D2D2D', borderRadius: '2px', animation: 'sitePulse 1.5s ease-in-out infinite' }} />
                </div>
              ))
            ) : (
              <>
                ...
                <AnimatedKPICard index={1} icon={<HiOutlineUsers size={24} />} color="#9CA3AF" label="ACTIVE WORKERS" value={stats?.workers?.active_workers || 0} subtitle={`of ${stats?.workers?.total_workers || 0} total workers`} onClick={() => navigate('/workers')} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '14px', marginBottom: '28px' }}>
            {statsLoading ? (
              ...
              <>
                ...
                <AnimatedKPICard index={9} icon={<HiOutlineUsers size={24} />} color="#9CA3AF" label="MANPOWER COST" value={fmt(stats?.costs?.manpower)} isMoney={true} subtitle="Labour wages total" onClick={() => navigate('/workers')} />
          ...
          <AnimatedItem delay={0.2}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              {/* Recent Projects */}
              <div style={{ background: '#1E1E1E', border: '1px solid #2D2D2D', borderRadius: '8px', padding: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '14px', fontWeight: '500', color: '#F5F5F4', textTransform: 'uppercase', letterSpacing: '1px', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {projectsLoading ? (
                    [1,2,3].map(i => (
                      <div key={i} style={{ padding: '12px 14px', background: '#262626', borderRadius: '6px' }}>
                    ...
                    recentProjects.map((p, i) => (
                      <div
                        ...
                        style={{
                          ...
                          padding: '12px 14px', background: '#262626', border: '1px solid transparent',
                        ...
                        onMouseLeave={e => { e.currentTarget.style.background = '#262626'; e.currentTarget.style.borderColor = 'transparent'; }}
                      >
                        <div>
                          <div style={{ fontWeight: '600', fontSize: '14px', color: '#F5F5F4' }}>{p.project_name}</div>
                          <div style={{ fontSize: '12px', color: '#9CA3AF', marginTop: '2px' }}>{p.location || 'No Location'}</div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          <span className={`badge badge-${p.status}`}>{p.status?.replace('_', ' ')}</span>
                          <span style={{ fontFamily: "'Roboto Mono', monospace", fontWeight: '500', fontSize: '13px', color: '#F5F5F4' }}>
                    ...
                    <p style={{ color: '#57534E', fontSize: '13px', margin: 0 }}>No recent projects</p>
                  )}
                </div>
              </div>
              ...
              <div style={{ background: '#1E1E1E', border: '1px solid #2D2D2D', borderRadius: '8px', padding: '24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {expensesLoading ? (
                    [1,2,3,4,5].map(i => (
                      <div key={i} style={{ padding: '12px 14px', background: '#262626', borderRadius: '6px' }}>
                        <div style={{ width: '50%', height: '14px', background: '#2D2D2D', borderRadius: '2px', animation: 'sitePulse 1.5s ease-in-out infinite' }} />
                      </div>
                    ))
                  ) : expensesError ? (
                    <p style={{ color: '#DC2626', fontSize: '13px', margin: 0 }}>Failed to load</p>
                    ...
                    <p style={{ color: '#57534E', fontSize: '13px', margin: 0 }}>No recent expenses</p>
                  )}
                </div>
              </div>
            </div>
